\chapter{Groups, Homomorphisms, and Quotients}\label{chap:groups-foundations}
...
\item \textbf{Associativity:} for all \(x,y,z\in G\),
      \[(xy)z=x(yz).\]
\item \textbf{Identity:} there is an element \(e\in G\) such that, for every
      \(x\in G\),
      \[ex=x=xe.\]
...
\begin{remark}
This example uses the familiar elementary notions of matrices and matrix
multiplication. Their systematic definition, operations, and relation to
linear transformations appear in
Section~\hyperref[sec:matrices-change-basis]{5.11}.
\end{remark}
...
\(n\mathbb Z\leq\mathbb Z\), whereas the positive integers are not a subgroup
of \((\mathbb Z,+)\), because they are not closed under additive inverses.
...
gives \(hh^{-1}=e\in H\).  Now take \(x=e\) and any \(y\in H\); the condition
...
\section{Homomorphisms, Isomorphisms, and Cayley Tables}

\begin{definition}
A \emph{group homomorphism} \(\varphi:G\to H\) satisfies
\(\varphi(xy)=\varphi(x)\varphi(y)\) for all \(x,y\in G\). Its
\emph{kernel} and \emph{image} are
\[
\ker\varphi=\{g\in G:\varphi(g)=e_H\},\qquad
\operatorname{im}\varphi=\{\varphi(g):g\in G\}.
\]
A homomorphism is \emph{injective} or \emph{surjective} in the ordinary
set-theoretic sense. A bijective homomorphism is an \emph{isomorphism}; in
that case we write \(G\cong H\).
\end{definition}

\begin{proposition}
A homomorphism \(\varphi:G\to H\) satisfies
\(\varphi(e_G)=e_H\), \(\varphi(g^{-1})=\varphi(g)^{-1}\), and
\(\varphi(g^n)=\varphi(g)^n\) for every \(n\in\mathbb Z\). Its kernel and
image are subgroups, and \(\varphi\) is injective if and only if
\(\ker\varphi=\{e_G\}\).
\end{proposition}
\begin{proof}
The equation \(\varphi(e_G)=\varphi(e_G)^2\) permits cancellation in \(H\)
and gives \(\varphi(e_G)=e_H\). Applying \(\varphi\) to
\(gg^{-1}=e_G\) proves the inverse formula, and the power formula follows by
iteration (and then by inverses for negative exponents). The subgroup test
applied to \(\varphi(x)\varphi(y)^{-1}=\varphi(xy^{-1})\) proves the image
claim; the same test proves the kernel claim. Finally,
\(\varphi(x)=\varphi(y)\) exactly when
\(\varphi(x^{-1}y)=e_H\), which gives the final assertion.
\end{proof}

\begin{theorem}[Two-sided inverse criterion]
A group homomorphism \(f:G\to H\) is an isomorphism if and only if there is a
group homomorphism \(g:H\to G\) such that
\[
g\circ f=\operatorname{id}_G,\qquad f\circ g=\operatorname{id}_H.
\]
\end{theorem}
\begin{proof}
If \(f\) is an isomorphism, its set-theoretic inverse \(f^{-1}\) is a
homomorphism: write \(u=f(x)\), \(v=f(y)\), and observe that
\(f^{-1}(uv)=f^{-1}(f(xy))=xy\). Take \(g=f^{-1}\). Conversely, the first
identity makes \(f\) injective and the second makes it surjective, so \(f\)
is an isomorphism.
\end{proof}

\begin{remark}
This is the group-theoretic instance of the categorical definition of an
isomorphism: a morphism possessing a two-sided inverse. No category theory is
needed here; the displayed equations are the whole content.
\end{remark}

For a finite group, a \emph{Cayley table} records the product of every ordered
pair of elements. The row labelled \(a\) and column labelled \(b\) meet in
\(ab\). The identity row and column reproduce the labels, and the location of
\(e\) in the row for \(a\) identifies \(a^{-1}\).

\begin{proposition}[Latin-square property]
Every row and every column of the Cayley table of a finite group contains each
group element exactly once.
\end{proposition}
\begin{proof}
If \(ax=ay\), cancellation gives \(x=y\); hence left multiplication by \(a\)
is injective and therefore bijective on a finite group. This proves the row
claim. Right cancellation gives the column claim.
\end{proof}

\begin{example}[Small Cayley tables]
\(C_n\) here denotes a cyclic group with \(n\) elements; this notation will be
developed formally in the next section.
\[
\begin{array}{c|cc}C_2&e&a\\\hline e&e&a\\a&a&e\end{array}\qquad
\begin{array}{c|ccc}C_3&e&a&a^2\\\hline e&e&a&a^2\\a&a&a^2&e\\a^2&a^2&e&a\end{array}
\]
\[
\begin{array}{c|cccc}C_4&e&a&a^2&a^3\\\hline
e&e&a&a^2&a^3\\a&a&a^2&a^3&e\\a^2&a^2&a^3&e&a\\a^3&a^3&e&a&a^2\end{array}
\quad
\begin{array}{c|cccc}C_2\times C_2&e&a&b&ab\\\hline
e&e&a&b&ab\\a&a&e&ab&b\\b&b&ab&e&a\\ab&ab&b&a&e\end{array}
\]
The final two tables show the two possible patterns for a group of order
four: one element may have order four, or every nonidentity element may have
order two.
\end{example}

\begin{proposition}[Groups of order at most four]
The group of order \(1\) is trivial; every group of order \(2\) or \(3\) is
cyclic; and every group of order \(4\) is isomorphic to \(C_4\) or to
\(C_2\times C_2\).
\end{proposition}
\begin{proof}
For order \(1\) there is only \(e\). If \(G=\{e,a\}\), then \(a^2=e\),
so \(G=\langle a\rangle\cong C_2\).

Let \(G=\{e,a,b\}\), where \(a\ne e\). If \(a^2=e\), then the row of
\(a\) has entries \(a\) and \(e\) in the columns \(e\) and \(a\). By the
Latin-square property its remaining entry is \(b\), so \(ab=b\); right
cancellation gives \(a=e\), a contradiction. Thus \(e,a,a^2\) are distinct,
exhaust \(G\), and \(G=\langle a\rangle\cong C_3\).

Now let \(|G|=4\). If some \(a\) has \(e,a,a^2,a^3\) all distinct, these
exhaust \(G\), and \(G\cong C_4\). Otherwise no nonidentity element has
order four. Nor can one have order three: if
\(H=\{e,a,a^2\}\) were a subgroup and \(b\notin H\), then \(b,ba,ba^2\)
would be distinct by cancellation and none could lie in \(H\), for
\(ba^i=a^j\) would imply \(b=a^{j-i}\in H\). This is impossible because
\(G\setminus H\) has only one element. Hence every nonidentity element has
order two. Choose distinct nonidentity elements \(a,b\). Then \(ab\) differs
from \(e,a,b\), so \(G=\{e,a,b,ab\}\), and
\(ab=(ab)^{-1}=b^{-1}a^{-1}=ba\). Thus the Cayley table is the displayed
table for \(C_2\times C_2\), and \(G\cong C_2\times C_2\).
\end{proof}

\begin{example}[The Rubik's Cube group]
Consider the transformations of a Rubik's Cube obtained from finite sequences
of legal face turns, with multiplication given by composition of transformations. Doing nothing is
the identity, move sequences compose associatively, and reversing each turn
in reverse order undoes a sequence. These transformations therefore form a
large finite nonabelian group. The example captures a central theme: groups
describe symmetries, reversible transformations, and reversible procedures.
\end{example}
...
and their inverses (including the empty product \(e\)). This formulation is
...
\(|g|\) is the least positive \(n\) with \(g^n=e\), if it exists.
...
\langle g^4\rangle,\langle g^6\rangle\), and \(\{e\}\), of orders
...
for which \(g^a=e\). If \(g\) has infinite order, this happens only for
\(a=0\). If \(|g|=n\), division with remainder shows that \(g^a=e\) if and
only if \(n\mid a\), so the kernel is \(n\mathbb Z\).  In the infinite-order
case, \(\varphi_g\) is one-to-one: \(g^a=g^b\) implies
\(g^{a-b}=e\), hence \(a=b\). It is onto \(\langle g\rangle\) by its
definition, and thus gives \(\mathbb Z\cong\langle g\rangle\).
If \(|g|=n\), the map
\[a+n\mathbb Z\longmapsto g^a\]
is well-defined because \(g^n=e\); it is a bijective homomorphism onto
...
\begin{example}[Roots of unity revisited]
The preliminary set \(\mu_n\) is closed under multiplication and inverses,
because \((zw)^n=z^nw^n=1\) and \((z^{-1})^n=1\). It is therefore a group
under multiplication. The \(n\) displayed roots are precisely
\(1,\zeta_n,\ldots,\zeta_n^{n-1}\), so
\(\mu_n=\langle\zeta_n\rangle\cong C_n\).
\end{example}
...
left in a product. Since \(s^2=e\) and \(r^n=e\), every product of rotations
...
The notation between angle brackets is a compact way to say that \(D_n\) is
generated by \(r\) and \(s\), and that these generators satisfy the displayed
relations. No general theory of presentations is needed here.

The smallest nonabelian dihedral example is \(D_3\), the symmetry group of an
equilateral triangle. Label its vertices \(1,2,3\), let \(r\) rotate through
\(120^\circ\), and let \(s\) reflect across the axis through vertex \(1\).
The six labelled pictures display the images of the original vertex labels:
\[
\begin{array}{ccc}
\begin{tikzpicture}[baseline=-.5ex,scale=.34]\draw (0,1)--(-.87,-.5)--(.87,-.5)--cycle;\node at (0,1.25){\scriptsize1};\node at (-1.08,-.65){\scriptsize2};\node at (1.08,-.65){\scriptsize3};\end{tikzpicture}&
\begin{tikzpicture}[baseline=-.5ex,scale=.34]\draw (0,1)--(-.87,-.5)--(.87,-.5)--cycle;\node at (0,1.25){\scriptsize3};\node at (-1.08,-.65){\scriptsize1};\node at (1.08,-.65){\scriptsize2};\end{tikzpicture}&
\begin{tikzpicture}[baseline=-.5ex,scale=.34]\draw (0,1)--(-.87,-.5)--(.87,-.5)--cycle;\node at (0,1.25){\scriptsize2};\node at (-1.08,-.65){\scriptsize3};\node at (1.08,-.65){\scriptsize1};\end{tikzpicture}\\[-2pt]
e&r&r^2\\[6pt]
\begin{tikzpicture}[baseline=-.5ex,scale=.34]\draw (0,1)--(-.87,-.5)--(.87,-.5)--cycle;\node at (0,1.25){\scriptsize1};\node at (-1.08,-.65){\scriptsize3};\node at (1.08,-.65){\scriptsize2};\end{tikzpicture}&
\begin{tikzpicture}[baseline=-.5ex,scale=.34]\draw (0,1)--(-.87,-.5)--(.87,-.5)--cycle;\node at (0,1.25){\scriptsize3};\node at (-1.08,-.65){\scriptsize2};\node at (1.08,-.65){\scriptsize1};\end{tikzpicture}&
\begin{tikzpicture}[baseline=-.5ex,scale=.34]\draw (0,1)--(-.87,-.5)--(.87,-.5)--cycle;\node at (0,1.25){\scriptsize2};\node at (-1.08,-.65){\scriptsize1};\node at (1.08,-.65){\scriptsize3};\end{tikzpicture}\\[-2pt]
s&sr&sr^2
\end{array}
\]
The first three symmetries are rotations and the last three are reflections.
Their effects on the vertices are summarized after permutation notation has
been introduced below.

The relation with permutation groups will be made precise after symmetric
groups and their notation have been introduced.

\section{Symmetric and Alternating Groups}

\begin{definition}
The \emph{symmetric group} \(S_n\) is the group of all bijections of the set
\(\{1,2,\ldots,n\}\), with composition as the operation and identity \(e\).
A \(k\)-\emph{cycle}
...
For example, the two transpositions \((12)\) and \((23)\) generate \(S_3\).
...
\begin{definition}
The \emph{alternating group} \(A_n\) is the kernel of the sign homomorphism,
\[
A_n=\ker(\operatorname{sgn})=\{\sigma\in S_n:\operatorname{sgn}(\sigma)=1\}.
\]
\end{definition}

The sign rule shows directly that the product and inverse of even permutations
are even, so \(A_n\) is a subgroup. For \(n\geq2\), the sign homomorphism is
surjective: a transposition has sign \(-1\). Its two fibers have the same
size, since multiplication by a fixed transposition bijects them. Therefore
\[
|A_n|=\frac{|S_n|}{2}=\frac{n!}{2}.
\]
This elementary count already shows that \(A_n\) has index two; normality will
also follow from the general kernel criterion below.
For \(S_3\), \(A_3=\{e,(123),(132)\}=\langle(123)\rangle\), and the three transpositions are
the odd permutations.  Later quotient theory will identify the two parity
classes with a copy of \(C_2\).

\begin{example}[The triangle group \(D_3\)]
Label an equilateral triangle cyclically by \(1,2,3\). Let \(r\) be the
counterclockwise rotation \(1\mapsto2\mapsto3\mapsto1\), and let \(s\) be the
reflection fixing vertex \(1\). With products evaluated from right to left,
the six geometric symmetries are:
\[
\begin{array}{c|c|ccc|c}
 &\text{description}&1&2&3&\text{permutation}\\\hline
e&\text{do nothing}&1&2&3&e\\
r&\text{rotation by }120^\circ&2&3&1&(123)\\
r^2&\text{rotation by }240^\circ&3&1&2&(132)\\
s&\text{reflection through vertex }1&1&3&2&(23)\\
sr&\text{reflection}&3&2&1&(13)\\
sr^2&\text{reflection}&2&1&3&(12)
\end{array}
\]
Now that \(S_3\) is defined, the final column identifies the six
rearrangements with permutations: \(r=(123)\), \(r^2=(132)\), and the three
reflections are the three transpositions. Thus the action on vertices defines
a homomorphism \(D_3\to S_3\) whose images are all six permutations. It is
injective because a symmetry fixing every vertex is the identity, and so
\(D_3\cong S_3\).
\end{example}

More generally, the action of \(D_n\) on the labelled vertices of a regular
\(n\)-gon gives a homomorphism \(D_n\hookrightarrow S_n\). It is injective:
a symmetry that fixes every vertex fixes the polygon pointwise and is the
identity.
...
\begin{corollary}
If \(G\) is finite, then \(g^{|G|}=e\) for every \(g\in G\).
\end{corollary}
\begin{proof}
The order of \(g\) divides \(|G|\).
\end{proof}

\begin{theorem}[Fermat's Little Theorem]\label{thm:fermat-little}
If \(p\) is prime and \(a\in\mathbb Z\) satisfies \(p\nmid a\), then
\[
a^{p-1}\equiv1\pmod p.
\]
...
\end{theorem}
\begin{proof}
The nonzero residue classes form the multiplicative group
\((\mathbb Z/p\mathbb Z)^\times\), which has \(p-1\) elements. Lagrange's
theorem applied to the class of \(a\) gives
\([a]^{p-1}=[1]\). If \(p\mid a\), the equivalent statement is immediate;
otherwise multiply the displayed congruence by \(a\).
\end{proof}
...
The kernel of a homomorphism is a subgroup, but it has a further invariance
property that makes quotient groups possible. This is the first important
connection between maps and quotient constructions.
...
\end{proposition}
\begin{proof}
For \(n\in\ker\varphi\), \(\varphi(gng^{-1})=\varphi(g)e_K\varphi(g)^{-1}=e_K\).
...
\(D_n=\langle r,s\mid r^n=s^2=e,\ srs^{-1}=r^{-1}\rangle\), the rotation
...
\begin{proofidea}
The quotient identifies \(g\) and \(gn\). The condition \(f(n)=e_H\) says
...
\(f(6k)=u^{6k}=e\), the subgroup \(6\mathbb Z\) is contained in \(\ker f\).
...
permutations \(A_3=\{e,(123),(132)\}\) and the three odd permutations. The
...
\(g^k=e\) and \(h^k=e\).  Thus its order is \(\operatorname{lcm}(m,n)\).
...
single element generated \(C_m\times C_n\). For a general element \((x,y)\),
\[
|(x,y)|=\operatorname{lcm}(|x|,|y|).
\]
Since \(|x|\mid m\) and \(|y|\mid n\), this order divides
\(\operatorname{lcm}(m,n)\). A generator would have order \(mn\), so this
can happen only when \(\operatorname{lcm}(m,n)=mn\), equivalently
\(\gcd(m,n)=1\).
\end{proof}
...
leaving Chapter~1. The same four questions organize each instance:
...
\[N=\{e,r,\ldots,r^{n-1}\}\quad\text{and}\quad sN=
\{s,sr,\ldots,sr^{n-1}\}.\]
...
\(|G|=|\operatorname{orb}(x)|\,|\operatorname{stab}(x)|\) later becomes a tool in Galois theory, representation
...
\section{Automorphisms and Inner Automorphisms}

An isomorphism compares two groups; an automorphism is a symmetry of one group
viewed as an algebraic structure. It rearranges the elements while preserving
every multiplication relation. Automorphisms therefore record the internal
symmetries of a group.

\begin{definition}
An \emph{automorphism} of a group \(G\) is an isomorphism
\(\varphi:G\to G\). The set of all automorphisms is denoted
\(\operatorname{Aut}(G)\).
\end{definition}
...
\end{proposition}
\begin{proof}
Composition of two automorphisms is an automorphism, the identity map
\(\operatorname{id}_G\) is an automorphism, and the inverse of an
automorphism is an automorphism by the two-sided inverse criterion.
Associativity is inherited from composition of functions.
\end{proof}

\begin{example}
Every automorphism of the additive group \(\mathbb Z\) is determined by the
image of \(1\), which must be a generator: either \(1\) or \(-1\). Hence
\(\operatorname{Aut}(\mathbb Z)\cong C_2\).

Likewise, if \(C_n=\langle a\rangle\), an automorphism is determined by
\(a\mapsto a^k\), and this is an automorphism exactly when \(a^k\) is also a
generator, equivalently when \(\gcd(k,n)=1\). Composition multiplies the
exponents modulo \(n\), so
\[
\operatorname{Aut}(C_n)\cong(\mathbb Z/n\mathbb Z)^\times.
\]
For instance, if \(C_6=\langle a\rangle\), its only generators are \(a\) and
\(a^5\). Thus its only automorphisms send \(a\) to \(a\) or to \(a^5\), and
\(\operatorname{Aut}(C_6)\cong C_2\). More generally, composing the maps
\(a\mapsto a^j\) and \(a\mapsto a^k\) sends \(a\) to \(a^{jk}\), which
explains the multiplication of exponents modulo \(n\).
\end{example}
...
\]
For \(g\in G\), define the \emph{inner automorphism}
\[
c_g:G\to G,\qquad c_g(x)=gxg^{-1}.
\]
For a concrete preview, conjugation by \((123)\) in \(S_3\) cycles the three
transpositions:
\begin{align*}
(123)(12)(123)^{-1}&=(23),\\
(123)(23)(123)^{-1}&=(13),\\
(123)(13)(123)^{-1}&=(12).
\end{align*}
Thus conjugation is itself a structure-preserving rearrangement, not merely a
calculation. In general, \(c_g\) is a homomorphism, and \(c_{g^{-1}}\) is
its inverse, so \(c_g\) is an automorphism. The assignment
\[
\Phi:G\longrightarrow\operatorname{Aut}(G),\qquad \Phi(g)=c_g
\]
is a homomorphism, because
\[
c_{gh}(x)=(gh)x(gh)^{-1}=g(hxh^{-1})g^{-1}
=(c_g\circ c_h)(x).
\]
Its kernel is
\[
\ker\Phi=\{g\in G:gxg^{-1}=x\text{ for every }x\in G\}=Z(G),
\]
and its image is the subgroup
\[
\operatorname{Inn}(G)=\{c_g:g\in G\}
\]
of \(\operatorname{Aut}(G)\). Thus central elements are exactly those that
act trivially by conjugation. The subgroup
\(\operatorname{Inn}(G)\leq\operatorname{Aut}(G)\) consists precisely of
the symmetries visible from conjugation inside \(G\). The First Isomorphism Theorem gives
\[
G/Z(G)\cong\operatorname{Inn}(G).
\]
Moreover, the \emph{canonical conjugation homomorphism} \(\Phi\) is injective
if and only if \(Z(G)=\{e\}\). In particular, a centerless group has the
canonical embedding \(G\hookrightarrow\operatorname{Aut}(G)\) supplied by
conjugation; this statement
concerns this specific embedding, not the possible existence of an unrelated
one.
...
\begin{proposition}
Suppose \(N,H\trianglelefteq G\), \(G=NH\), and \(N\cap H=\{e\}\).  Then
\(G\cong N\times H\).
\end{proposition}
\begin{proof}
...
\end{exercise}
\begin{exercise}[Core]
Suppose \(H\leq G\) is the unique subgroup of \(G\) of order \(|H|\). Prove
