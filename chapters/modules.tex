...
Division by every nonzero scalar is exactly what permits the replacement
arguments behind ordinary linear algebra. The results in this section are
therefore theorems about vector spaces, not assertions about arbitrary
modules.

\begin{definition}
An \emph{\(F\)-vector space} is an \(F\)-module. A \emph{subspace} of a
vector space is an \(F\)-submodule. A finite expression
\(a_1v_1+\cdots+a_rv_r\), with \(a_i\in F\), is a \emph{linear
combination}. For a subset \(S\subseteq V\), its \emph{span}
\[\operatorname{span}(S)=\left\{\sum_{i=1}^r a_is_i:r\geq0,\ a_i\in F,\ s_i\in S\right\}
\]
is the smallest subspace containing \(S\). The set \(S\) \emph{spans}
\(V\) when \(\operatorname{span}(S)=V\). It is \emph{linearly
independent} when every finite relation among distinct members of \(S\) has
all coefficients zero. A \emph{basis} is an independent spanning set.
An \emph{ordered basis} is a basis displayed as an ordered finite list.
Finally, \(V\) is \emph{finite-dimensional} if it has a finite spanning set.
\end{definition}

The definition of span makes clear both why it is a subspace and why it is
the smallest one: sums and scalar multiples of finite linear combinations
are again finite linear combinations, while any subspace containing \(S\)
contains every such combination. The module notions of generation and basis
agree with these definitions, but fields allow the stronger conclusions below.

\begin{lemma}[Exchange lemma]
Let \(v_1,\ldots,v_r\) be linearly independent vectors in \(V\), and let
\(w_1,\ldots,w_s\) span \(V\). Then \(r\leq s\). Moreover, one can replace
\(r\) of the \(w_j\)'s by the \(v_i\)'s and still obtain a spanning list.
\end{lemma}
\begin{proof}
Write
\[
v_1=\sum_j a_jw_j.
\]
Some \(a_j\ne0\), so field division permits us
to solve for that \(w_j\). Replacing this \(w_j\) by \(v_1\) still leaves a
spanning list. Inductively, suppose \(v_1,\ldots,v_{i-1}\) have replaced
distinct \(w\)'s. Express \(v_i\) in the resulting spanning list. If every
coefficient of an unreplaced \(w\) were zero, \(v_i\) would lie in the span
of \(v_1,\ldots,v_{i-1}\), contradicting independence. Hence an unreplaced
coefficient is nonzero and may be solved for. After \(r\) steps there must
have been at least \(r\) original \(w\)'s.
\end{proof}

\begin{theorem}[Finite-dimensional basis theorem]
If \(V\) has a finite spanning set, then every independent finite list can
be extended to a basis, every finite spanning list contains a basis, and any
two bases have the same finite cardinality.
\end{theorem}
\begin{proof}
Starting with an independent list, adjoin vectors from a fixed finite
spanning list whenever they are not already in its span. The resulting list
remains independent and eventually spans, hence is a basis. Starting with a
finite spanning list, discard a vector whenever it lies in the span of the
others; the finite process ends at an independent spanning list. Applying the
exchange lemma in both directions to two bases proves equality of their
cardinalities.
\end{proof}
...
\(W\) not in its span whenever one exists. The exchange lemma, applied to
the resulting independent list and a fixed basis of \(V\), shows that at
most \(\dim_FV\) choices can be made. When the process stops its list spans
\(W\), so it is a basis. The same exchange lemma gives the inequality.
\end{proof}

The common cardinality of a finite basis is now well-defined and is called
the \emph{dimension} \(\dim_FV\). In particular, a basis expansion exists
because the basis spans and is unique because the basis is independent.

\section{Invariant Basis Number and Finite Free Rank}
For vector spaces, the preceding theorem proves that the size of a finite
basis is intrinsic. A finite free module over a general ring needs a separate
argument. This distinction is essential in Chapter~7, where the free part of
a module over a PID must have a uniquely determined rank.
...
because \(\mathfrak m\) is maximal, so the finite-dimensional basis theorem
just proved gives \(m=n\).
\end{proof}
...
\section{Matrices and Change of Basis}
...
bases have been chosen. The transpose simply interchanges the row and column
indices. Directly from the entries,
\[
(A^t)^t=A,
\qquad
(A+B)^t=A^t+B^t,
...
(AB)^t=B^tA^t
\]
whenever the indicated sums and products are defined. If \(R\) is
...
\(x=(x_1,\ldots,x_n)^t\), define matrix--vector multiplication by
\[
(Ax)_i=\sum_{j=1}^n a_{ij}x_j.
\]
\[
\begin{pmatrix}a_{11}&\cdots&a_{1n}\\a_{21}&\cdots&a_{2n}\\\vdots&&\vdots\\a_{m1}&\cdots&a_{mn}\end{pmatrix}
\begin{pmatrix}x_1\\x_2\\\vdots\\x_n\end{pmatrix}
=\begin{pmatrix}a_{11}x_1+\cdots+a_{1n}x_n\\a_{21}x_1+\cdots+a_{2n}x_n\\\vdots\\a_{m1}x_1+\cdots+a_{mn}x_n\end{pmatrix}.
\]
For instance,
\[
...
\]
The same row-by-column rule represents composition:
\[
\begin{pmatrix}2&-1\\3&4\end{pmatrix}
\begin{pmatrix}1&0\\2&1\end{pmatrix}
=\begin{pmatrix}0&-1\\11&4\end{pmatrix}.
\]
Thus the \((i,j)\)-entry is the dot product of row \(i\) of the left factor
with column \(j\) of the right factor, exactly as required by composition.
If \(T:V\to W\), \(\beta=(v_1,\ldots,v_n)\), and
\(\gamma=(w_1,\ldots,w_m)\) is an ordered basis of \(W\), write
\[
...
\[
[T]^\gamma_\beta=(a_{ij})\in M_{m\times n}(F).
\]
Thus its \(j\)-th column is \([T(v_j)]_\gamma\); its entries depend on both
chosen ordered bases, though \(T\) itself does not. If
\[
v=\sum_jx_jv_j,
\]
linearity gives
\[
T(v)=\sum_jx_jT(v_j)=\sum_i\left(\sum_j a_{ij}x_j\right)w_i,
\]
so, exactly by the displayed definition of matrix--vector multiplication,
\[
[T(v)]_\gamma=[T]^\gamma_\beta[v]_\beta. \tag{6.1}
\]

For \(A=(a_{ij})\in M_{m\times n}(F)\) and
\(B=(b_{jk})\in M_{n\times r}(F)\), define
\[
AB=(c_{ik})\in M_{m\times r}(F),\qquad c_{ik}=\sum_{j=1}^n a_{ij}b_{jk}.
\]
This is row-by-column multiplication; equivalently, the \(k\)-th column of
\(AB\) is \(A\) times the \(k\)-th column of \(B\). The dimension rule is
\((m\times n)(n\times r)\to(m\times r)\). A direct finite-sum calculation
shows associativity: the \((i,\ell)\)-entries of \((AB)C\) and \(A(BC)\)
are respectively
\[
\sum_{j,k}a_{ij}b_{jk}c_{k\ell}
\]
with the two possible parentheses,
which agree in the field \(F\). Applying (6.1) twice therefore proves
\[
[S\circ T]^\delta_\beta=[S]^\delta_\gamma[T]^\gamma_\beta.
\]

The \emph{identity matrix} \(I_n=(\delta_{ij})\) has entries \(1\) on the
diagonal and \(0\) elsewhere. The defining sums immediately give
\(AI_n=A\) and \(I_mA=A\) for compatible matrices. A square matrix \(A\)
is \emph{invertible} if a square matrix \(B\) satisfies \(AB=BA=I\); such a
\(B\), if it exists, is unique, since
\(B=B(AC)=(BA)C=C\), and is denoted \(A^{-1}\). Associativity also gives
\((AB)^{-1}=B^{-1}A^{-1}\). Formula (6.1) and the composition formula show
that a square matrix is invertible exactly when its associated linear map is
an automorphism.
