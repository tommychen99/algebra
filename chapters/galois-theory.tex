...
\begin{equation}\label{eq:galois-fixed-field}
K/F\text{ is normal}\quad\Longleftrightarrow\quad
\sigma(K)=K\text{ for every }\sigma\in G.
\end{equation}
...
\]
The finite correspondence and \eqref{eq:galois-fixed-field} show that \(H\) is normal exactly when
...
Chapter~7 used the Fundamental Theorem of Algebra to obtain an eigenvalue of a
...
\begin{unremark}
Chapter~7 met the Fundamental Theorem of Algebra through complex linear
...
\descriptivesubsection{Basic Point-Set Topology}
...
\descriptivesubsection{Topological Groups}
...
\descriptivesubsection{Profinite Groups}
...
\]
is closed.  We have
\begin{equation}\label{eq:infinite-galois-fixed-field}
E^{\operatorname{Gal}(E/K)}=K.
\end{equation}
...
\descriptivesubsection{Groups, Actions, and Linear Algebra}
...
\descriptivesubsection{Rings, Local Structure, and Geometry}
...
\descriptivesubsection{Modules, Exactness, and Multilinear Algebra}
...
\descriptivesubsection{Fields, Arithmetic, and Galois Symmetry}
...
\descriptivesubsection{A Synthesis}
