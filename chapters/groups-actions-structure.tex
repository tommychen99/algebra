...
\begin{definition}
Let \(G\) be a group and let \(X\) be a set. A \emph{left action} of \(G\)
on \(X\) is a map \(G\times X\to X\), written \((g,x)\mapsto g\cdot x\),
satisfying the following two requirements:
\begin{enumerate}
\setlength{\itemsep}{5pt}
\item the group identity does nothing: \(e\cdot x=x\) for all \(x\in X\);
\item successive motions combine according to multiplication:
      \((gh)\cdot x=g\cdot(h\cdot x)\).
\end{enumerate}
For \(x\in X\), its \emph{orbit} and \emph{stabilizer} are
\[
\operatorname{orb}(x)=\{g\cdot x:g\in G\}\subseteq X,\qquad
\operatorname{stab}(x)=\{g\in G:g\cdot x=x\}\leq G.
\]
For \(g\in G\), its \emph{fixed-point set} is
\[
\operatorname{fix}(g)=\{x\in X:g\cdot x=x\}\subseteq X.
\]
\footnote{Common alternatives are \(Gx\) and \(\operatorname{Orb}_G(x)\) for
the orbit, and \(G_x\), \(\operatorname{Stab}_G(x)\), and
\(\operatorname{Stab}(x)\) for the stabilizer. These notes consistently use
\(\operatorname{orb}(x)\), \(\operatorname{stab}(x)\), and
\(\operatorname{fix}(g)\).}
\end{definition}
...
\end{proposition}
\begin{proof}
For the stabilizer, \(e\cdot x=x\), so \(e\in\operatorname{stab}(x)\). If
\(g,h\in\operatorname{stab}(x)\),
...
\(g^{-1}\) yields \(x=g^{-1}h\cdot y\). Hence
\(x\in\operatorname{orb}(y)\), and applying group elements shows
\(\operatorname{orb}(x)\subseteq\operatorname{orb}(y)\); symmetry gives
equality. Finally,
...
permutation is the identity map, then evaluating it at \(e\) gives \(g=e\).
Its kernel is therefore trivial, so the homomorphism is injective.
\end{proof}

\begin{example}
For the left action of \(G\) on itself, the orbit of \(e\) is all of \(G\)
...
For every group action, the map
\[
G/\operatorname{stab}(x)\longrightarrow\operatorname{orb}(x),\qquad
g\operatorname{stab}(x)\longmapsto g\cdot x
\]
is a bijection. Thus
\(|\operatorname{orb}(x)|=[G:\operatorname{stab}(x)]\) whenever these
cardinalities are considered; if \(G\) is finite, then
\[
|G|=|\operatorname{orb}(x)|\,|\operatorname{stab}(x)|.
\]
\end{theorem}
\begin{proofidea}
The cosets of \(\operatorname{stab}(x)\) are exactly the different ways to
move \(x\). Two
elements \(g,h\) send \(x\) to the same point precisely when
\(h^{-1}g\) fixes \(x\), which is the equivalence relation defining the cosets.
\end{proofidea}
\begin{proof}
Define \(\Phi:G/\operatorname{stab}(x)\to\operatorname{orb}(x)\) by
\(\Phi(g\operatorname{stab}(x))=g\cdot x\). To check well-definedness,
suppose \(g\operatorname{stab}(x)=h\operatorname{stab}(x)\). Then
\(h^{-1}g\in\operatorname{stab}(x)\), whence
\(g\cdot x=h\cdot((h^{-1}g)\cdot x)=h\cdot x\).  Conversely, if
\(g\cdot x=h\cdot x\), applying \(h^{-1}\) gives
\((h^{-1}g)\cdot x=x\); thus \(h^{-1}g\in\operatorname{stab}(x)\) and
\(g\operatorname{stab}(x)=h\operatorname{stab}(x)\). This
proves injectivity at the same time.  Surjectivity holds because every point
of the orbit has the form \(g\cdot x\). Therefore
\(|\operatorname{orb}(x)|=[G:\operatorname{stab}(x)]\), and Lagrange's
theorem yields \(|G|=|\operatorname{orb}(x)|\,|\operatorname{stab}(x)|\).
\end{proof}
...
Let a finite group \(G\) act on a finite set \(X\). Then the number of orbits is
\[
...
Each of these sets is a subgroup of \(G\). Recall that the center
\(Z(G)\), introduced with inner automorphisms, is the collection of elements
that commute with all of \(G\). For example, the subgroup test
...
where \(\operatorname{fix}(P)=\{x\in X:g\cdot x=x\text{ for every }g\in P\}\).
\end{lemma}
\begin{proof}
...
gives \(|\operatorname{fix}(P)|\equiv p+1\equiv1\pmod p\), so there is a global fixed point.
...
centralizer of \((12)\) is \(\{e,(12)\}\), so orbit--stabilizer gives a
...
the identity because a cyclic rotation conjugates the total product by its first entry.
\begin{proofidea}
...
the repeated tuples, which encode elements satisfying \(g^p=e\).
\end{proofidea}
\begin{proof}
Let \(X\) be the set of \(p\)-tuples \((g_1,\ldots,g_p)\) with
\(g_1\cdots g_p=e\). Its first \(p-1\) entries may be chosen freely, so
...
\((g,\ldots,g)\) with \(g^p=e\). Thus the number of such \(g\) is divisible
by \(p\). It includes \(e\), so there is a nonidentity one; its order divides
...
\begin{proposition}
This operation makes \(N\rtimes_\alpha H\) a group; \(N\times\{e_H\}\) is
normal, \(\{e_N\}\times H\) is a subgroup, and they have trivial intersection.
\end{proposition}
\begin{proof}
The identity is \((e_N,e_H)\). Since every \(\alpha(h)\) is an automorphism,
...
\[
 (n,h)(n',e_H)(n,h)^{-1}=
 \bigl(n\alpha(h)(n')n^{-1},e_H\bigr)\in N\times\{e_H\},
\]
so \(N\times\{e_H\}\) is normal. The two subgroups meet only in
\((e_N,e_H)\).
\end{proof}
...
Here the notation means that \(N\to G\) is injective, \(G\to H\) is
surjective, and the image of \(N\to G\) is the kernel of \(G\to H\).
The word ``split'' means that the quotient map \(q:G\to H\) has a
group-homomorphic
section \(s:H\to G\). Then \(q\) restricted to \(s(H)\) is inverse to \(s\),
so \(N\cap s(H)=\{e\}\), where \(N=\ker q\). Moreover, if \(g\in G\) and
\(h=q(g)\), then
\[
q\bigl(gs(h)^{-1}\bigr)=hh^{-1}=e_H,
\]
...
but it explains why semidirect products are ubiquitous. The full
module-theoretic language of exact sequences is developed in Chapter~5.

\begin{example}
Let \(C_2=\langle s\rangle\) act on \(C_3=\langle r\rangle\) by inversion,
\(srs^{-1}=r^{-1}\).  Then \(C_3\rtimes C_2\) has presentation
\(\langle r,s\mid r^3=s^2=e,\ srs^{-1}=r^{-1}\rangle\), and the map sending
...
introduced in Chapter~1 provide a geometric model for every semidirect product
...
layers. A nontrivial group is \emph{simple} if it has no nontrivial proper
normal subgroup. A \emph{normal series} is a finite chain
\[
G=G_0\trianglerighteq G_1\trianglerighteq\cdots
\trianglerighteq G_r=\{e\},
\qquad G_{i+1}\trianglelefteq G_i,
\]
and its factors are \(G_i/G_{i+1}\). A simple group cannot
...
\begin{example}
The chain \(S_3\trianglerighteq A_3\trianglerighteq\{e\}\) is a composition
...
\end{proposition}
\begin{proof}
Induct on the group order.  If \(G\ne\{e\}\), choose a maximal proper normal
...
\end{theorem}
\begin{proof}
We argue by induction on \(|G|\).  There is nothing to prove when \(G\) is
trivial.
Write the two series as
\[
 G=G_0\trianglerighteq G_1=N\trianglerighteq\cdots\trianglerighteq\{e\},
 \qquad
 G=H_0\trianglerighteq H_1=M\trianglerighteq\cdots\trianglerighteq\{e\}.
\]
...
Choose a composition series from \(I\) down to \(\{e\}\).  Because \(N/I\) is
...
of \(x\) and \(y\) to commute: \([x,y]=e\) if and only if \(xy=yx\). Indeed,
\(xyx^{-1}y^{-1}=e\) is equivalent, after multiplying on the right by \(yx\),
...
\(f:G\to A\) and \(A\) is abelian, then \(f([x,y])=e_A\) for all \(x,y\), hence
...
\begin{theorem}
A group \(G\) is solvable if and only if \(G^{(r)}=\{e\}\) for some \(r\geq0\).
\end{theorem}
\begin{proof}
If the derived series reaches \(\{e\}\), its successive quotients are abelian by
the preceding proposition.  Conversely, let
\(G=G_0\trianglerighteq\cdots\trianglerighteq G_r=\{e\}\) have abelian factors.
Inductively, \(G^{(i)}\leq G_i\): if \(G^{(i)}\leq G_i\), then the image of
\(G^{(i)}\) in the abelian group \(G_i/G_{i+1}\) has trivial derived subgroup,
so \(G^{(i+1)}\leq G_{i+1}\).  Hence \(G^{(r)}=\{e\}\).
\end{proof}
...
\begin{definition}
Set \(Z_0(G)=\{e\}\) and define the \emph{upper central series} inductively by
\[Z_{i+1}(G)/Z_i(G)=Z\bigl(G/Z_i(G)\bigr).\]
The group \(G\) is \emph{nilpotent} if \(Z_c(G)=G\) for some \(c\).  The least
such \(c\) is its nilpotence class (unless \(G\) is trivial).
\end{definition}

Equivalently, a nilpotent group admits a finite chain
\[
\{e\}=N_0\leq N_1\leq\cdots\leq N_c=G
\]
...
\[
 \{e\}=\overline Z_0\leq\overline Z_1\leq\cdots\leq\overline Z_c=G/C.
\]
Let \(\pi:G\to G/C\) be the quotient map and define
\(Z_i=\pi^{-1}(\overline Z_i)\). Then
\(Z_0=C\), and the chain
\[
 \{e\}\leq C=Z_0\leq Z_1\leq\cdots\leq Z_c=G
\]
is central: if \(z\in Z_{i+1}\) and \(g\in G\), then
\(\pi(z)\overline Z_i\) is central in
\((G/C)/\overline Z_i\). Hence
\(\pi([z,g])\in\overline Z_i\), so
\([z,g]\in\pi^{-1}(\overline Z_i)=Z_i\). The first step
...
G_r=\{e\}\) be a solvable series, and let \(H\leq G\).  Intersecting gives
\[
 H=H\cap G_0\trianglerighteq H\cap G_1\trianglerighteq\cdots
 \trianglerighteq H\cap G_r=\{e\}.
\]
...
q(G_r)=\{e\}\) has normal terms.  Each nontrivial factor is an image of a
...
\begin{theorem}[Nonsolvability of alternating and symmetric groups]
For every \(n\geq5\), \(A_n\) is not solvable. Consequently \(S_n\) is not
solvable for \(n\geq5\).
\end{theorem}
\begin{proof}
As a standard external input, we use that \(A_n\) is a nonabelian simple
group for \(n\geq5\). Its proof is outside the present
Sylow-and-composition-series scope, and this result is not used in the later
internal dependency chain. Since \(A_n\) is nonabelian, its derived subgroup
\([A_n,A_n]\) is
nontrivial. It is normal, so simplicity gives
\([A_n,A_n]=A_n\). The derived series therefore never reaches the trivial
group, and \(A_n\) is not solvable. If \(S_n\) were solvable, its subgroup
\(A_n\) would be solvable by the preceding proposition, a contradiction.
\end{proof}
...
\(1\) gives
\[|S_3|=|\operatorname{orb}(1)|\,|\operatorname{stab}(1)|=3\cdot2.\]
The stabilizer is \(\{e,(23)\}\), a copy of \(C_2\). Thus the action turns a
...
The square group \(D_4=\langle r,s\mid r^4=s^2=e,\ srs^{-1}=r^{-1}\rangle\)
...
The center of \(D_4\) is \(\{e,r^2\}\). Since the quotient by this subgroup
is abelian, the chain \(\{e\}\leq\langle r^2\rangle\leq D_4\) is a central series.
