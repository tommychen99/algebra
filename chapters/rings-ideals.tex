...
nonzero \(b\). A \emph{domain} is a nonzero commutative ring with identity
and no zero divisors; a \emph{field} is a nonzero commutative ring in which
...
\[
I+J=\{i+j:i\in I,j\in J\},\quad I\cap J,\quad
IJ=\left\{\sum i_kj_k:i_k\in I,j_k\in J\right\}.
\]
They are ideals. The sum is the smallest ideal containing both; the
intersection is their largest common subideal. Always \(IJ\subseteq I\cap J\).
...
\phantomsection\label{thm:maximal-ideal-existence}
...
\end{example}
The corresponding statement for finitely many pairwise comaximal ideals
...
Fraction fields and localizations at primes are two further instances.  If
\(R\) is a domain, then
\[\operatorname{Frac}(R)\cong(R\setminus\{0\})^{-1}R;\]
in particular \(\mathbb Q\cong(\mathbb Z\setminus\{0\})^{-1}\mathbb Z\).
Thus the ordinary fraction-field construction inverts every nonzero element.
At the other extreme, localization at a prime ideal will invert only the
elements outside that prime.
...
\end{definition}
The conditions in the definition are forced by fraction arithmetic: if
\(s,t\) are allowed denominators, then
